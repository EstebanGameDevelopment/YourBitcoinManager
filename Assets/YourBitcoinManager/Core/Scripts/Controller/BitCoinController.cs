		...
		*/
		private void BroadcastTransaction(QBitNinjaClient _clientQBitNinja, Transaction _customerTransaction)
		{
			...
			else
			{
				...
				BasicEventController.Instance.DispatchBasicEvent(EVENT_BITCOINCONTROLLER_TRANSACTION_DONE, false, LanguageController.Instance.GetText("text.locations.failed.to.broadcast.transaction.to.network"));
			}
		}
		...
		*/
		public void Pay(string _currentPrivateKey,  string _publicKey, string _title, decimal _finalValueBitcoins, decimal _finalFeeAmount)
		{
			...
			if (balanceCurrent < m_finalValueBitcoins + m_finalFeeAmount)
			{
				...
				BasicEventController.Instance.DispatchBasicEvent(EVENT_BITCOINCONTROLLER_TRANSACTION_DONE, false, LanguageController.Instance.GetText("controller.bitcoin.not.enough.funds.to.perform", balanceCurrencyTrimmed, transactionCurrencyTrimmed));
				return;
			}
