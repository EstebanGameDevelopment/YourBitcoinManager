...
namespace YourBitcoinManager
{
	 ...
	 */
	public class FileElementManagerView: ScreenBaseView, IBasicScreenView
	{
		// ----------------------------------------------
		// CONSTANTS
		// ----------------------------------------------	
		...
		{
			m_itemsEachPage = (int)_list[0];
			m_data = (List<ItemMultiObjects>)_list[1];
			m_slotPrefab = (GameObject)_list[2];
		...
		public void ClearCurrentGameObject(bool _resetPage)
		{
			for (int i = 0; i < m_gameObjects.Count; i++)
			{
				...
				{
					m_gameObjects[i].GetComponent<FileElementView>().Destroy();
			...
			if ((m_buttonNext != null) && (m_buttonPrevious != null) && (m_data.Count > m_itemsEachPage))
			{
				...
				for (i = initialItem; i < finalItem; i++)
				{
					if (i < m_data.Count)
					{
						...
						newSlot.GetComponent<FileElementView>().Initialize(m_data[i]);
