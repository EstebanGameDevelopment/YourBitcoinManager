...
namespace YourBitcoinManager
{
	...
	public class ScreenBitcoinSendView : ScreenBaseView, IBasicScreenView
	{
		...
		public void Initialize(params object[] _list)
		{
			...
			m_messageInput.text = LanguageController.Instance.GetText("screen.send.explain.please");
		...
		{
			ScreenController.Instance.CreateNewScreen(ScreenBitcoinPrivateKeyView.SCREEN_NAME, TypePreviousActionEnum.HIDE_CURRENT_SCREEN, true, false);
		}
		...
		private void OnBackButton()
		{
			if (HasChanged)
			{
				...
				ScreenController.Instance.CreateNewInformationScreen(ScreenInformationView.SCREEN_CONFIRMATION, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, warning, description, null, SUB_EVENT_SCREENBITCOIN_CONFIRMATION_EXIT_TRANSACTION);
			}
			else
			{
				Destroy();
		...
		{
			ScreenController.Instance.CreateNewScreen(ScreenSelectAddressFromView.SCREEN_NAME, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, false);
		}
		...
		private void OnExecutePayment()
		{
			...
			if (!m_validPublicAddressToSend)
			{
				ScreenController.Instance.CreateNewInformationScreen(ScreenInformationView.SCREEN_INFORMATION, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, LanguageController.Instance.GetText("message.error"), LanguageController.Instance.GetText("screen.bitcoin.send.no.valid.address.to.send"), null, "");
			}
			else
			{
				...
				{					
					...
					{
						ScreenController.Instance.CreateNewInformationScreen(ScreenInformationView.SCREEN_INFORMATION, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, LanguageController.Instance.GetText("message.error"), LanguageController.Instance.GetText("screen.bitcoin.send.amount.no.zero"), null, "");						
					}
					else
					{
		...
		{
			ScreenController.Instance.CreateNewScreen(ScreenTransactionSummaryView.SCREEN_NAME, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, false, m_amountInBitcoins, m_feeInBitcoins, m_currencySelected, BitCoinController.Instance.AddressToLabel(m_publicAddressToSend), m_messageInput.text);
		}
		...
		private void OnBasicEvent(string _nameEvent, params object[] _list)
		{
			...
			{
				ScreenController.Instance.DestroyScreensOverlay();
				ScreenController.Instance.CreateNewInformationScreen(ScreenInformationView.SCREEN_WAIT, TypePreviousActionEnum.KEEP_CURRENT_SCREEN, LanguageController.Instance.GetText("message.info"), LanguageController.Instance.GetText("message.please.wait"), null, "");
